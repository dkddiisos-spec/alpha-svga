...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS site_config (id INT PRIMARY KEY, logoUrl TEXT, vodafoneNumber TEXT, usdtAddress TEXT, binanceId TEXT, visitors INT, downloads INT, sales INT)");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS license_keys (id VARCHAR(50) PRIMARY KEY, `key` VARCHAR(100) UNIQUE, createdAt BIGINT, expiresAt BIGINT, isUsed INT, activatedAt BIGINT)");

    $stmt = $pdo->query("SELECT COUNT(*) FROM site_config");
    if ($stmt->fetchColumn() == 0) {
        $pdo->exec("INSERT INTO site_config VALUES (1, '', '01102930761', '', '', 0, 0, 0)");
    }
...
switch ($action) {
    case 'upload':
        ...
        break;
    case 'getConfig':
        echo json_encode($pdo->query("SELECT * FROM site_config WHERE id=1")->fetch());
        break;
    case 'updateConfig':
        $stmt = $pdo->prepare("UPDATE site_config SET logoUrl=?, vodafoneNumber=?, usdtAddress=?, binanceId=? WHERE id=1");
        $stmt->execute([$data['logoUrl'], $data['vodafoneNumber'], $data['usdtAddress'], $data['binanceId']]);
    ...
    case 'getCategories':
        ...
        break;
    case 'getProducts':
        echo json_encode($pdo->query("SELECT * FROM products")->fetchAll());
        break;
    case 'deleteProduct':
        ...
        break;
    case 'getOrders':
        echo json_encode($pdo->query("SELECT * FROM orders ORDER BY timestamp DESC")->fetchAll());
        break;
    case 'addOrder':
        $stmt = $pdo->prepare("INSERT INTO orders VALUES (?,?,?,?,?,?,?,?)");
        $stmt->execute([$data['id'], $data['productId'], $data['status'], $data['paymentMethod'], $data['customerName'], $data['proofImageUrl'], $data['timestamp'], 0]);
        echo json_encode(["success" => true]);
        break;
    case 'approveOrder':
    ...
    case 'markDownloaded':
        ...
        break;
    case 'getKeys':
        echo json_encode($pdo->query("SELECT * FROM license_keys ORDER BY createdAt DESC")->fetchAll());
        break;
    case 'addKey':
        $stmt = $pdo->prepare("INSERT INTO license_keys VALUES (?,?,?,?,?,?)");
        $stmt->execute([$data['id'], $data['key'], $data['createdAt'], null, 0, null]);
        echo json_encode(["success" => true]);
        break;
    case 'activateKey':
        $stmt = $pdo->prepare("UPDATE license_keys SET isUsed=1, activatedAt=?, expiresAt=? WHERE `key`=?");
        $stmt->execute([$data['activatedAt'], $data['expiresAt'], $data['key']]);
        echo json_encode(["success" => true]);
        break;
    case 'deleteKey':
    ...
    case 'deleteCategory':
        ...
        break;
    default:
        echo json_encode(["error" => "الإجراء غير صالح"]);
        break;
}
?>